...
// cancels any pending close, without the timeout for events that don't need the hover like closing the X icon, drop down link, click outside
 ...
 //clean up for the timer
  ...
  const hasImageLinks = activeItem?.children?.some((child) => (child.image?.length ?? 0) > 0) ?? false;

  // has items on child 
  const ImageHasChildren = activeItem?.children?.some((child) => !!child.image?.[0]?.url) ?? false;
